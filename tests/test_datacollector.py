...
def test_DataCollector_reset_state_when_file_exists(geo):
    outdir = Path("testdir")

    simcardems.DataCollector(outdir, geo=geo)

    with mock.patch("simcardems.utils.remove_file") as remove_file_mock:
        collector = simcardems.DataCollector(outdir, geo=geo, reset_state=True)
    remove_file_mock.assert_called()
    assert Path(collector.results_file).is_file()
    shutil.rmtree(outdir)


def test_DataCollector_not_reset_state_when_file_exists(geo):
    outdir = Path("testdir")

    simcardems.DataCollector(outdir, geo=geo)

    with mock.patch("simcardems.utils.remove_file") as remove_file_mock:
        collector = simcardems.DataCollector(outdir, geo=geo, reset_state=False)
    remove_file_mock.assert_not_called()
    assert Path(collector.results_file).is_file()
    shutil.rmtree(outdir)


def test_DataCollector_create_file_with_mesh(geo):
    outdir = Path("testdir")

    collector = simcardems.DataCollector(outdir, geo=geo)
    assert Path(collector.results_file).is_file()
...
def test_DataCollector_register(geo):
...
def test_DataLoader_load_empty_files_raises_ValueError(geo):
    outdir = Path("testdir")
