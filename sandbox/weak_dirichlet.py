...
# Boundary conditions
weak_dirichlet = False

# Penalty terms
beta = dolfin.Constant(10.0)
h = dolfin.MaxFacetEdgeLength(mesh)

left = dolfin.CompiledSubDomain("near(x[0], 0) && on_boundary")
marker_left = dolfin.MeshFunction("size_t", mesh, mesh.topology().dim() - 1, 0)
left.mark(marker_left, 1)
ds_dir = dolfin.Measure("ds", domain=mesh, subdomain_data=marker_left, subdomain_id=1)

if weak_dirichlet:
    # Add penalty terms
    u_D = dolfin.Constant((0.0, 0.0, 0.0))
    R += (beta / h) * dolfin.inner(u, v) * ds_dir - (beta / h) * dolfin.inner(
        u_D,
        v,
    ) * ds_dir
    # TODO : Add symetrization terms ?
    bc = None
else:
...
u, p = state.split()
if weak_dirichlet:
    dolfin.File("u_weak.pvd") << u
else:
    dolfin.File("u_strong.pvd") << u
