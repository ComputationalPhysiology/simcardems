...
class TorLandFull(BaseCellModel):
    ...
    def _I(self, v, s, time):
        ...
        PhiCaL_ss = (
            4.0 * vffrt * (gamma_cai * cass * ufl.exp(2.0 * vfrt) - gamma_cao * cao) / (-1.0 + ufl.exp(2.0 * vfrt))
        )
        PhiCaNa_ss = (
            1.0 * vffrt * (gamma_nai * nass * ufl.exp(1.0 * vfrt) - gamma_nao * nao) / (-1.0 + ufl.exp(1.0 * vfrt))
        )
        PhiCaK_ss = 1.0 * vffrt * (gamma_ki * kss * ufl.exp(1.0 * vfrt) - gamma_kao * ko) / (-1.0 + ufl.exp(1.0 * vfrt))
        PhiCaL_i = (
            4.0 * vffrt * (gamma_cai * cai * ufl.exp(2.0 * vfrt) - gamma_cao * cao) / (-1.0 + ufl.exp(2.0 * vfrt))
        )
        PhiCaNa_i = (
            1.0 * vffrt * (gamma_nai * nai * ufl.exp(1.0 * vfrt) - gamma_nao * nao) / (-1.0 + ufl.exp(1.0 * vfrt))
        )
        ...
        ICaL_tot_ss = (1.0 - fICaLp) * ((1.0 - nca) * f + fca * jca * nca) * PCa * PhiCaL_ss * d + (
            (1.0 - nca) * fp + fcap * jca * nca
        ) * PCap * PhiCaL_ss * d * fICaLp
        ICaNa_tot_ss = (1.0 - fICaLp) * ((1.0 - nca) * f + fca * jca * nca) * PCaNa * PhiCaNa_ss * d + (
            (1.0 - nca) * fp + fcap * jca * nca
        ) * PCaNap * PhiCaNa_ss * d * fICaLp
        ICaK_tot_ss = (1.0 - fICaLp) * ((1.0 - nca) * f + fca * jca * nca) * PCaK * PhiCaK_ss * d + (
            (1.0 - nca) * fp + fcap * jca * nca
        ) * PCaKp * PhiCaK_ss * d * fICaLp

        ICaL_tot_i = (1.0 - fICaLp) * ((1.0 - nca) * f + fca * jca * nca) * PCa * PhiCaL_i * d + (
            (1.0 - nca) * fp + fcap * jca * nca
        ) * PCap * PhiCaL_i * d * fICaLp
        ICaNa_tot_i = (1.0 - fICaLp) * ((1.0 - nca) * f + fca * jca * nca) * PCaNa * PhiCaNa_i * d + (
            (1.0 - nca) * fp + fcap * jca * nca
        ) * PCaNap * PhiCaNa_i * d * fICaLp
        ICaK_tot_i = (1.0 - fICaLp) * ((1.0 - nca) * f + fca * jca * nca) * PCaK * PhiCaK_i * d + (
            (1.0 - nca) * fp + fcap * jca * nca
        ...
        bK1 = (15.72 * ufl.exp(0.0674 * (v - EK - 3.257)) + ufl.exp(0.0618 * (v - EK - 594.31))) / (
        ...
        a1 = (
            ...
            * ufl.elem_pow(nai / Knai, 3.0)
            / (-1.0 + ufl.elem_pow(1.0 + ki / Kki, 2.0) + ufl.elem_pow(1.0 + nai / Knai, 3.0))
        )
        b1 = MgADP * k1m
        a2 = k2p
        b2 = (
            ...
            * ufl.elem_pow(nao / Knao, 3.0)
            / (-1.0 + ufl.elem_pow(1.0 + ko / Kko, 2.0) + ufl.elem_pow(1.0 + nao / Knao, 3.0))
        )
        a3 = (
            k3p
        ...
        INab = PNab * scale_drug_INab * scale_popu_GbNa * (-nao + ufl.exp(vfrt) * nai) * vffrt / (-1.0 + ufl.exp(vfrt))
        ...
        Isac_P_k = ufl.conditional(
            ufl.lt(self.lmbda, 1.0),
            0.0,
            Gsac_k * ((self.lmbda - 1.0) / (lambda_max - 1.0)) * (1.0 / (1.0 + ufl.exp((19.05 - v) / (29.98)))),
        )
    ...
    def F(self, v, s, time=None):
        ...
        aj = ufl.conditional(
            ufl.lt(v, -40.0),
            ((-25428 * ufl.exp(0.2444 * v) - 6.948e-6 * ufl.exp(-0.04391 * v)) * (v + 37.78))
        ...
        tff = 7.0 + 1.0 / (0.03325075244518792 * ufl.exp(0.1 * v) + 0.0006090087745647571 * ufl.exp(-0.1 * v))
        tfs = 1000.0 + 1.0 / (
            1.0027667890106652e-05 * ufl.exp(-0.25 * v) + 8.053415618124885e-05 * ufl.exp(0.16666666666666666 * v)
        )
        Afs = 1.0 - Aff
        ...
        tfcas = 100.0 + 1.0 / (0.00012 * ufl.exp(0.14285714285714285 * v) + 0.00012 * ufl.exp(-0.3333333333333333 * v))
        Afcaf = 0.3 + 0.6 / (1.0 + 0.36787944117144233 * ufl.exp(0.1 * v))
        Afcas = 1.0 - Afcaf
        ...
        F_expressions[26] = kr_c0 * alpha_c0 + kr_c2 * beta_c1 - kr_c1 * (beta_c0 + alpha_c1)
        F_expressions[27] = (
            kr_c1 * alpha_c1 + kr_o * beta_c2o + kr_i * beta_c2i - kr_c2 * (beta_c1 + alpha_c2o + alpha_c2i)
        )
        F_expressions[28] = kr_c2 * alpha_c2o + kr_i * beta_oi - kr_o * (beta_c2o + alpha_oi)
        F_expressions[29] = kr_c2 * alpha_c2i + kr_o * alpha_oi - kr_i * (beta_c2i + beta_oi)
        ...
        xs2ss = xs1ss
        txs2 = 1.0 / (
            0.0022561357010639103 * ufl.exp(-0.03225806451612903 * v) + 0.0008208499862389881 * ufl.exp(0.05 * v)
        )
        ...
        F_expressions[34] = JdiffK * vss / vmyo + (
            -ICaK_i - Isac_P_k - IK1 - IKb - IKr - IKs - Istim - Ito - Isac_P_ns / 3.0 + 2.0 * INaK
        ) * Acap / (F * vmyo)
        F_expressions[35] = -JdiffK - Acap * ICaK_ss / (F * vss)
        Bcass = 1.0 / (
            1.0 + BSLmax * KmBSL * ufl.elem_pow(KmBSL + cass, -2.0) + BSRmax * KmBSR * ufl.elem_pow(KmBSR + cass, -2.0)
        )
        F_expressions[36] = (-Jdiff + Jrel * vjsr / vss + 0.5 * (-ICaL_ss + 2.0 * INaCa_ss) * Acap / (F * vss)) * Bcass
        ...
        kwu = -kws * scale_popu_kws + (kuw * scale_popu_kuw) * (-1.0 + 1.0 / (rw * scale_popu_rw))
        ksu = kws * scale_popu_kws * rw * scale_popu_rw * (-1.0 + 1.0 / (rs * scale_popu_rs))
        ...
        cat50 = (cat50_ref * scale_popu_CaT50ref + Beta1 * (-1.0 + lambda_min12)) * HF_scaling_cat50_ref
        CaTrpn = ufl.conditional(ufl.lt(CaTrpn, 0.0), 0.0, CaTrpn)
        F_expressions[41] = (
            ktrpn
            * scale_popu_kTRPN
            * (-CaTrpn + ufl.elem_pow(1000.0 * cai / cat50, ntrpn * scale_popu_nTRPN) * (1.0 - CaTrpn))
        )
        kb = (
            ku
            * scale_popu_ku
            * ufl.elem_pow(Trpn50 * scale_popu_TRPN50, (ntm * scale_popu_nTm))
            / (1.0 - (rs * scale_popu_rs) - rw * scale_popu_rw * (1.0 - (rs * scale_popu_rs)))
        )
        ...
        F_expressions[42] = (
            ...
            * XU
            * kb
            - ku * scale_popu_ku * ufl.elem_pow(CaTrpn, (ntm * scale_popu_nTm) / 2.0) * TmB
        )
