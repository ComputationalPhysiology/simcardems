...
class ORdmmLandExplicit(BaseCellModel):
    ...
    def _I(self, v, s, time):
        ...
        PhiCaL = 4.0 * (-0.341 * cao + cass * ufl.exp(2.0 * vfrt)) * vffrt / (-1.0 + ufl.exp(2.0 * vfrt))
        PhiCaNa = 1.0 * (-0.75 * nao + 0.75 * ufl.exp(1.0 * vfrt) * nass) * vffrt / (-1.0 + ufl.exp(1.0 * vfrt))
        PhiCaK = 1.0 * (-0.75 * ko + 0.75 * ufl.exp(1.0 * vfrt) * kss) * vffrt / (-1.0 + ufl.exp(1.0 * vfrt))
        ...
        rk1 = 1.0 / (1.0 + 69220.6322106767 * ufl.exp(0.10534077741493732 * v - 0.27388602127883704 * ko))
    ...
    def F(self, v, s, time=None):
        ...
        td = 0.6 + 1.0 / (3.5254214873653824 * ufl.exp(0.09 * v) + 0.7408182206817179 * ufl.exp(-0.05 * v))
        ...
        GKs = 0.0034 * scale_IKs * scale_drug_IKs * scale_popu_GKs
        IKs = (-EKs + v) * GKs * KsCa * xs1 * xs2
        xk1ss = 1.0 / (1.0 + ufl.exp((-144.59 - v - 2.5538 * ko) / (3.8115 + 1.5692 * ko)))
        txk1 = 122.2 / (
        ...
        a_rel = 0.5 * bt
        Jrel_inf = -ICaL * a_rel / (1.0 + 25.62890625 * HF_scaling_Jrel_inf * ufl.elem_pow(1.0 / cajsr, 8.0))
        ...
        btp = 1.25 * bt
        a_relp = 0.5 * btp
        Jrel_infp = -ICaL * a_relp / (1.0 + 25.62890625 * HF_scaling_Jrel_inf * ufl.elem_pow(1.0 / cajsr, 8.0))
        ...
        F_expressions[36] = (-Jdiff + Jrel * vjsr / vss + 0.5 * (-ICaL + 2.0 * INaCa_ss) * Acap / (F * vss)) * Bcass
        ...
        F_expressions[40] = kuw * scale_popu_kuw * XU - kws * scale_popu_kws * XW - XW * gammawu - XW * kwu
